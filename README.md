...
Скриншоты работающего приложения. Каталог и результаты настоящие для предоставленного датасета; имена анонимизированы, синтетические профили отмечены в интерфейсе.
...
```bash
git clone --branch feat/catalog-ai https://github.com/BAITC-Hacks/hack-d3e8cd85-soda.git
cd hack-d3e8cd85-soda
docker compose up -d --wait db
go run .
```

Откройте **[localhost:8080](http://localhost:8080)**. Команда `go run .` остаётся работать в терминале. Если репозиторий уже открыт, выполняйте последние две команды из его корня.
...
AI участвует **до выдачи карточек**: разбирается в идее мероприятия, уточняет специализацию и преобразует рассказ в поля и пожелания. Проверку обязательных условий выполняет Go. Поэтому красивое описание профиля не позволяет обойти бюджет или календарь.
...
Во время поиска по подтверждённым условиям внешняя языковая модель **не вызывается**. Ориентир ответа — до 10 секунд; это цель задания, а не заявленная гарантия под нагрузкой.

### Почему повтор возвращает тот же порядок
...
В AI-консультанте есть кнопки готовых примеров: они открывают **подтверждение**, а не обходят проверки. Таблица ниже рассчитана по исходному каталогу; для точного порядка оставьте пожелания пустыми.
...
Ответ содержит `status`, `cards`, число совпадений, причины исключения, способ сортировки, версию данных и отдельные альтернативы. Описание всех шести API-маршрутов — в [справочнике](docs/TECHNICAL.md#api).
...
Каталог — **[исходный CSV организаторов](data/catalog.csv)**: 66 профилей, 17 категорий. Это 50 профилей Алматы, 15 Астаны и один с городом «Зарубежье». Данные из сокращённого HTML-превью не используются.
...
PostgreSQL хранит профили, занятость, цитаты и происхождение. Повторный запуск не затирает изменения, а каждый новый запрос читает актуальный каталог.
